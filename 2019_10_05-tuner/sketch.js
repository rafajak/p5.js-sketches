...
function draw() {
	if (display_text == true) {
		...
		if (image_cache.length < 10) {
			...
			text("The strange images you'll see will be generated by your own computer.", width / 2 - 210, height / 2 + 20 * 4)
			text("It will use a compact generative neural network model (DCGAN).", width / 2 - 210, height / 2 + 20 * 5)
			text("The model was trained to draw pictures of human faces.", width / 2 - 210, height / 2 + 20 * 6)
			text("The results are horrible, and so is the accompanying music I made.", width / 2 - 210, height / 2 + 20 * 7)
			text("(I hope you like it as much as I do)", width / 2 - 210, height / 2 + 22 * 9)
			textSize(12);
	...
	if ((image_cache.length > 0) & (level > 0.31)) {
		{
			for (i = 0; i < 3; i++) {
				for (j = 0; j < 3; j++) {
...
// dcgan_highres.generate(displayImage)
